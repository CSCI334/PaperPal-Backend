...
export default class ValidatePhase extends BaseMiddleware {
    ...
    public handler = async (req: Request, res: Response, next: NextFunction) => {
        ...
        if(res.locals.phaseType === "CURRENT") {
            if(!phases.includes(currentPhase)) next(new ForbiddenException(`Conference is not currently in ${phases} phase`));
        }
        else if(res.locals.phaseType === "PASSED") { 
            if(!(currentPhase > phases[0])) next(new ForbiddenException(`Conference has not yet passed ${phases} phase`));
        }
        next();
    };
}